...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        speed = np.float16(3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2))
        steering = np.int16(c.steer*90)
        throttle = np.float16(c.throttle*100)
        brake = np.uint8(c.brake*100)
        ...
        #data_frame = pd.DataFrame({'Speed':[speed],'Steering':[steering],'Throttle':[throttle],'Brake': [brake],'LaneDetection': [lanedetection]})
        #print(lanedetection)
        ...
        # ],
        # "value": [speed,steering,throttle,brake,lanedetection],
        # "delay": [0,0,0,0,0]
        # })
        # 데이터프레임에 데이터 추가
